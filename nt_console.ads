...
--  Mail:        jdijk@acm.org
--
--  Copyright (c) Jerry van Dijk, 1997, 1998, 1999 Billie Holidaystraat 28 2324
--  LK LEIDEN THE NETHERLANDS tel int + 31 71 531 43 65
--
--  Permission granted to use for any purpose, provided this copyright remains
--  attached and unmodified.
--
...
package NT_Console is
   ...
   subtype X_Pos is Natural range 0 .. 1024;
   subtype Y_Pos is Natural range 0 .. 1024;
   ...
   wx : Integer := 192; -- Largeur de l'écran
   wy : Integer := 73; -- Hauteur de l'écran

   --     XS1 : Integer := 1; -- Coordonnées  point Haut - Gauche de l'écran
   --dans le
   --                          --paysage
   --     YS1 : Integer := 1;
   --     XS2 : Integer := 80; -- Coordonnées  point Bas - Droit de l'écran
   --dans le
   --                          --paysage
   --     YS2 : Integer := 50;
   protected type Screen is
      entry Goto_XY (X : in Integer; Y : Integer);
      entry clearscreen;
      entry Ecrit
        (x  : Integer;
         y  : Integer;
   ...
   -- SCREEN CONTROL --
   --------------------
   procedure Screen_Dimension; -- ajoute le 22 mai 2009 par Yves
